      ...
      <main class="inr-page Training-page Recruit-page" id="ResearchPage">
		...
		//   if(!empty($section1Image)){
         ...
         <section class="training-prog-sec space pt-0" >
			...
			<?php
//                    $temp = array();
//			        $rs = new WP_Query(array('post_type' => 'research','post_status' =>'publish','posts_per_page' => 2,'orderby' => 'date','order' => 'ASC'));
//				    if ( $rs -> have_posts() ) {
//
...
//				    $temp = get_the_ID();
                $featuredResearch = get_field('featured_industry_research');
             ?>

                  <?php foreach ($featuredResearch as $oneResearch) : ?>
                      <?php //print_r($oneResearch); ?>
                      <div class="col-lg-3 col-md-6 col-12">
                         <div class="training-prog-box">
                            <h4><?=$oneResearch->post_title;?></h4>
                            <p><?php echo ($oneResearch->post_excerpt)? $oneResearch->post_excerpt : '';?></p>
                            <a href="/research/<?= $oneResearch->post_name; ?>" class="btn-border">Learn More</a>
                         </div>
                      </div>
                  <?php endforeach; ?>
