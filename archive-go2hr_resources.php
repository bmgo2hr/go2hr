...
get_header();?>
  ...
  <!-- header end -->
      <main class="inr-page Training-page Recruit-page" id="ExploreResources">
	   ...
	   <?php get_template_part( 'template-parts/content/inner-page-banner', null, array('option_prefix' => 'explore') ); ?>
					...
					<div class="col-12">
                        ...
                        <?php    $topic_id ='';
                                 if(isset($_GET['topic']) && !empty($_GET['topic'])){
									...
									<li class="<?= $selected ?>"><a href="/explore-all-resources">All Topics</a></li>
									<?php
                                ...
                                // Reorder the terms
                                 ...
                                 ksort($new_eventCat);
                                 foreach($new_eventCat as $cat){
                                    if($topic_id == $cat->term_id){ $selected = 'selected'; }else{ $selected = ''; } ?>
                                 <li class="<?= $selected ?>">
                                    <a href="<?php echo home_url(); ?>/explore-all-resources/?topic=<?php echo $cat->term_id; ?>">
                                       <?=$cat->name;?>
                                    </a>
                                 </li>
										<?php } //endif
      ...
      <?php       // If any search form is submitted then create the tax query for data
                  ...
                  if(isset($_GET['topic']) && !empty($_GET['topic'])){
                     ...
                     $tax_array[2]['operator'] =  'IN';
                  }
                  if(isset($_GET['resource_subtopic']) && !empty($_GET['resource_subtopic'])){
                     $subtopic = $_GET['resource_subtopic'];
                     $subtopic_array = explode(',',$subtopic);
                     $tax_array['relation'] = 'AND';
                     $tax_array[3]['taxonomy'] = 'subtopic';
                     $tax_array[3]['field'] = 'term_id';
                     $tax_array[3]['terms'] = $subtopic_array;
                     $tax_array[3]['operator'] =  'IN';
                  }
                  if(isset($_GET['tag']) && !empty($_GET['tag'])){
                     $tag = $_GET['tag'];
                     $tag_array = explode(',',$tag);
                     $tax_array['relation'] = 'AND';
                     $tax_array[4]['taxonomy'] = 'resource_tag';
                     $tax_array[4]['field'] = 'term_id';
                     $tax_array[4]['terms'] = $tag_array;
                     $tax_array[4]['operator'] =  'IN';
                  }
                        ...
                        <div id="accordion" class="filter-accordion">
                           ...
                           $cats = get_object_taxonomies( 'go2hr_resources');// print_r($cats);
                              ...
                              ?>
                                 ...
                                 <div id="collapse<?=$flg;?>" class="collapse <?php echo ($flg == 1)? 'show':'show';?>" aria-labelledby="heading<?=$flg;?>"  style=""> <!--data-parent="#accordion"-->
                                    <div class="card-body">
...
function formUrl(){
   ...
   jQuery('.card').each(function(){
      ...
      // can't use the same value as the post_type, otherwise it will be not found with the parameter
      if(cat=='subtopic'){
         cat = 'resource_subtopic';
      }
      url = url+cat+'=';
   ...
   console.log(url);
   window.location.href = '<?php echo home_url(); ?>/explore-all-resources/'+url;
   //get_sortData(acat);
}
