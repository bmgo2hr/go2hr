...
class G2hr_Jobs {
  ...
  public function init_ajax_update() {
    ...
    public function delete() {
        ...
        $status = $type == "not-expired" ? "job_archived" : "job_deleted";
        $success_message = $type == "not-expired" ? "Successfully archived." : "Successfully deleted.";
    ...
    */
    public function add($data = false, $draft = false, $preview = false) {
        ...
        if ($_POST['data']['use_different_address']) {
            $_POST['data']['address1'] = $_POST['data']['different_address1'];
        }
        ...
        $job_data = [
            ...
            'publish'                   =>    'publish',
            'use_different_address'     =>    $_POST['data']['use_different_address'],
        ];
...
//      'end_date'                    =>    filter_var($data->end_date, FILTER_SANITIZE_STRING)
    ...
    $message = $_POST['data']['repost'] == 1 ? 'Job has been successfully reposted.' : 'Job has been successfully updated.';
    wp_send_json_success(
      array(
        'redirect_url'    =>    $redirect_url,
        'title'            =>    __( 'Done!', 'go2hr' ),
        'message'        =>    __( $message, 'go2hr' ),
      ),
      200
    );
